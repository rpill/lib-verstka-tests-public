...
const compareLayout = async (htmlPath, options, handler) => {
    var _a, _b;
    const { fullPage = true, canonicalImage = 'layout-canonical.jpg', reqPercentage = 10, } = options;
    fs_1.default.appendFileSync(htmlPath, videoStopScript);
    const { browser, page } = await (0, puppeteer_1.launchBrowser)(htmlPath, options.browserOptions);
