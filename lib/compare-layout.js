...
const compareLayout = async (htmlPath, options, handler) => {
    var _a;
    const { fullPage = true, canonicalImage = 'layout-canonical.jpg', reqPercentage = 10, browserOptions, } = options;
    fs_1.default.appendFileSync(htmlPath, videoStopScript);
    const { browser, page } = await (0, puppeteer_1.launchBrowser)(htmlPath, browserOptions);
    if (handler.onBeforeScreenshot) {
        await handler.onBeforeScreenshot(page);
    }
    ...
    fs_1.default.writeFileSync(outputImage, data.getBuffer ? (_a = data.getBuffer) === null || _a === void 0 ? void 0 : _a.call(data, true) : '');
